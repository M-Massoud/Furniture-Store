...
router.route("/users")
    .all(authMW)
    .get(adminAuthorizationMW, controller.getAllUsers)
    ...
    .post([
        ...
        body("password").isStrongPassword().withMessage("password should be strong password").isLength({ min: 6 }).withMessage("password length should be 6 at least  numbers"),
        ...
        check('payment.cardNumber').isNumeric().withMessage("card number should be number").isLength({ min: 12, max: 12 }).withMessage("card number length should be 12 numbers"),
    ],
        validationMW,
        controller.createUser)
    .put(adminAuthorizationMW, [
...
router.route("/user/:id")
    .all(authMW)
    .get(adminOrUserByIdAuthorizationMW, [
        param("id").isNumeric().withMessage("user id should be number"),
    ],
        validationMW,
        controller.getUserById)
    .put(adminOrUserByIdAuthorizationMW, [
        param("id").isNumeric().withMessage("user id should be number"),
        body("firstName").optional().isAlpha().withMessage("user first name should be characters").isLength({ min: 1, max: 12, }).withMessage("firstName length should be between 1 and 12 numbers"),
        body("lastName").optional().isAlpha().withMessage("user last name should be characters").isLength({ min: 1, max: 12, }).withMessage("lastName length should be between 1 and 12 numbers"),
        body("mobile").optional().isMobilePhone().withMessage("mobile should be mobile numbers").isLength({ min: 10, max: 14, }).withMessage("mobile length should be between 10 and 14  numbers"),
    ...
    .delete(adminOrUserByIdAuthorizationMW, [
        param("id").isNumeric().withMessage("user id should be number"),
    ],
        validationMW,
