...
router
  ...
  .post(
    [
      body('firstName')
        .isAlpha()
        .withMessage('user first name should be characters')
        .isLength({ min: 1, max: 12 })
        .withMessage('firstName length should be between 1 and 12 numbers'),
      body('lastName')
        .isAlpha()
        .withMessage('user last name should be characters')
        .isLength({ min: 1, max: 12 })
        .withMessage('lastName length should be between 1 and 12 numbers'),
      body('email')
        .isEmail()
        .withMessage('email should be with standared email form'),
      body('password')
        .isStrongPassword()
        .withMessage(
          'user Password shoud be at least 8 characters, with upper case,lower case, special character and numbers'
        ),
      body('mobile')
        .isMobilePhone()
        .withMessage('mobile should be mobile numbers')
      ...
      body('wishList')
        .optional()
        .isArray({ type: Number })
        .withMessage('wishList should be array of productId'),

      body('payment')
        .optional()
        .isObject()
        .withMessage('payment should be object'),
      check('payment.cardType')
        .isIn(['visa', 'mastercard', 'meza'])
        .withMessage(
          "card type should be one of these ['visa','mastercard','meza']"
        ),
      check('payment.cardNumber')
        .isNumeric()
        .withMessage('card number should be number')
        .isLength({ min: 12, max: 12 })
        .withMessage('card number length should be 12 numbers'),
    ],
    validationMW,
  ...
  .put(
    ...
    [
      ...
      body('orders')
        .optional()
        .isArray({ type: Object })
        .withMessage('orders should be array of objects'),
      check('orders.*.id')
        .optional()
        .isNumeric()
        .withMessage('order id should be number'),
      check('orders.*.productId')
        .optional()
        .isNumeric()
        .withMessage('orders productId should be number'),
      check('orders.*.productTitle')
        .optional()
        .isString()
        .withMessage('orders productTitle should be string'),
    ],
    validationMW,
...
router
  ...
  .get(
    authMW,
    adminOrUserByIdAuthorizationMW,
